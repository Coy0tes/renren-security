$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/resumetraining/list',
        datatype: "json",
        colModel: [			
			...
			{ label: '学历', name: 'degree', index: 'degree', width: 80 }, 			
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.resumeTraining.id == null ? "sys/resumetraining/save" : "sys/resumetraining/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
                    contentType: "application/json",
                    data: JSON.stringify(vm.resumeTraining),
                    success: function(r){
                        if(r.code === 0){
		...
		del: function (event) {
            ...
            layer.confirm('确定要删除选中的记录？', {
               ...
               if(!lock) {
		            ...
		            $.ajax({
                        type: "POST",
                        url: baseURL + "sys/resumetraining/delete",
                        contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "sys/resumetraining/info/"+id, function(r){
                vm.resumeTraining = r.resumeTraining;
            });
		},
